...
class ChallengeLeaderboardManager {
	...
	GetTopPlayersLeaderboard(limit, callback) {
		...
		var params = [limit];
		var sql = "select count(*) attempts, username from challenge_attempts join users u on challenge_attempts.user_id = u.id group by uid, username order by attempts desc limit ?;";
		...
		this.dbm.ParameterizedQuery(sql, params, function(results) {
			...
			var challengeLeaderboard = new ChallengeLeaderboard("Most Frequent Players");
			results.forEach(function(entry) {
				username = entry.username;
				if (self.UsernameIsClean(username) == false) {
					username = "*****";
				...
				var challengeLeaderboardEntry = new ChallengeLeaderboardEntry(username, attempt_id, attempts);
				challengeLeaderboard.AddLeaderboardEntry(challengeLeaderboardEntry);
			});
